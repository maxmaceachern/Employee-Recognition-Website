...
namespace NolveltyApp.Controllers
{
    public class UserController : Controller
    {
        ...
        public string CSVrole { get; private set; }
        public string CSVsearch { get; private set; }
        ...
        // GET: User
        public ActionResult Index(string roleString, string searchString, bool yesNo = false)
        {
            if (yesNo == true)
            {
                CSV(roleString, searchString);
            }


            var RoleList = new List<string>();
            var RoleQry = (from r in context.Roles
                           orderby r.Name
                           select r.Name);

            RoleList.AddRange(RoleQry.Distinct());
            ViewBag.roleString = new SelectList(RoleList);
            ...
            {
                users = users.Where(s => s.RoleName == roleString);
            }
            return View(users);
        }
        ...
        public bool CSV(string roleString, string searchString)
        {
            ...
            if (!String.IsNullOrEmpty(roleString))
            {
                items = items.Where(s => s.RoleName == roleString);
            }
